...
pub fn compile_binary(
    ...
    use BinaryOperator::*;
    let new_expr: Expr = match node.operator {
        Minus => Expr::BinaryExpr {
        ...
        LessThan => Expr::BinaryExpr {
            left: Box::new(lhs_numeric),
            op: Operator::Lt,
            right: Box::new(rhs_numeric),
        },
        LessThanEqual => Expr::BinaryExpr {
            left: Box::new(lhs_numeric),
            op: Operator::LtEq,
            right: Box::new(rhs_numeric),
        },
        GreaterThan => Expr::BinaryExpr {
            left: Box::new(lhs_numeric),
            op: Operator::Gt,
            right: Box::new(rhs_numeric),
        },
        GreaterThanEqual => Expr::BinaryExpr {
            left: Box::new(lhs_numeric),
            op: Operator::GtEq,
            right: Box::new(rhs_numeric),
        },
