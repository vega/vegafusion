...
) -> Result<HashMap<ScopedVariable, DataSupported>> {
    ...
    for node_index in nodes {
        ...
        if !matches!(node_supported, DataSupported::Unsupported) {
            ...
            let all_parents_supported = data_graph
                ...
                .all(|parent_var| match supported_vars.get(&parent_var) {
...
pub fn build_data_graph(
    chart_spec: &ChartSpec,
) -> Result<DiGraph<(ScopedVariable, DataSupported), ()>> {
