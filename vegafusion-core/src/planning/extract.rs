...
pub fn extract_server_data(client_spec: &mut ChartSpec, task_scope: &mut TaskScope) -> Result<ChartSpec> {
...
impl <'a> ExtractServerDataVisitor<'a> {
    pub fn new(supported_vars: HashMap<ScopedVariable, DataSupported>, task_scope: &'a mut TaskScope) -> Self {
        Self {
    ...
    fn visit_data(&mut self, data: &mut DataSpec, scope: &[u32]) -> Result<()> {
        ...
        match self.supported_vars.get(&data_var) {
            Some(DataSupported::PartiallySupported) => {
                ...
                // transform.
                let server_tx: Vec<_> = data.transform.iter().cloned().take_while(
                    |tx| tx.supported()
                ).collect();

                let client_tx: Vec<_> = data.transform.iter().cloned().skip_while(
                    |tx| tx.supported()
                ).collect();
                ...
                self.task_scope.add_variable(
                    &Variable::new_data(&server_name), scope
                )?;
