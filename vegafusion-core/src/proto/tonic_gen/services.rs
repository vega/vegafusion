...
pub mod vega_fusion_runtime_server {
    ...
    #[async_trait]
    pub trait VegaFusionRuntime: std::marker::Send + std::marker::Sync + 'static {
    ...
    #[derive(Debug)]
    pub struct VegaFusionRuntimeServer<T> {
        inner: Arc<T>,
    ...
    impl<T> VegaFusionRuntimeServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for VegaFusionRuntimeServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "services.VegaFusionRuntime";
    impl<T> tonic::server::NamedService for VegaFusionRuntimeServer<T> {
