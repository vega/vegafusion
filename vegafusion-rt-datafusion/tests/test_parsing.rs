...
#[test]
fn test_parsing() {
    let v = ESTreeExpression::Literal(Literal { value: LiteralValue::Number(23.3), raw: "23.30".to_string() });
    let s = serde_json::to_string(&v).unwrap();
    println!("s: {}", s);
    // let v = serde_json::from_str("{}")
    // println!("Here");

    let d: ESTreeExpression = serde_json::from_str(r#"{"type":"Literal","value":23.3,"raw":"23.30"}"#).unwrap();
    println!("d: {:?}", d);
}
