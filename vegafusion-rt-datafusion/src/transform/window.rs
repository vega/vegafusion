...
use vegafusion_core::proto::gen::transforms::{SortOrder, Window, window_transform_op, AggregateOp, WindowOp};
...
use datafusion::logical_plan::window_frames::{WindowFrame, WindowFrameBound, WindowFrameUnits};
...
impl TransformTrait for Window {
    async fn eval(
        ...
        let order_by: Vec<_> = self.sort_fields.iter().zip(&self.sort).map(|(field, order)| {
            Expr::Sort {
        ...
        let partition_by: Vec<_> = self.groupby.iter().map(|group| {
            col(group)
        }).collect();
        ...
        let window_exprs: Vec<_> = self.ops.iter().zip(&self.fields).enumerate().map(|(i, (op, field)) | {
            let (window_fn, args) = match op.op.as_ref().unwrap() {
                window_transform_op::Op::AggregateOp(op) => {
                    ...
                    let (agg_fn, arg) = match op {
                        Count => {
                            (aggregates::AggregateFunction::Count, lit(true))
                        },
                        Sum => {
                            (aggregates::AggregateFunction::Sum, col(field))
                        },
                        Mean | Average => {
                            (aggregates::AggregateFunction::Avg, col(field))
                        },
                        Min => {
                            (aggregates::AggregateFunction::Min, col(field))
                        },
                        Max => {
                            (aggregates::AggregateFunction::Max, col(field))
                        },
                        Values => {
                            (aggregates::AggregateFunction::ArrayAgg, col(field))
                        }
                        _ => {
        ...
        let mut selections: Vec<_> = dataframe.schema().fields().iter().map(|f| col(f.field().name())).collect();
