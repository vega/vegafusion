...
use datafusion::arrow::array::{ArrayRef, Date64Array, Int64Array, TimestampMillisecondArray, TimestampMillisecondBuilder};
...
fn make_timeunit_start_udf(units_mask: &[bool], in_local: bool) -> ScalarUDF {
    ...
    let timeunit = move |args: &[ArrayRef]| {
        ...
        let result_array = if matches!(dtype, DataType::Date64) {
            // Input UTC
            let array = arg.as_any().downcast_ref::<Date64Array>().unwrap();
            let result_array: Date64Array = if in_local {
            ...
            if in_local {
                ...
                let result_array: TimestampMillisecondArray = unary(array, |value| {
                    perform_timeunit_start_from_in_local(value, units_mask.as_slice())
                });
                Arc::new(result_array) as ArrayRef
            } else {
                // Input Local, convert Local to UTC, then compute timeunit in UTC, and return in UTC
                let tz = Utc;
                let result_array: Date64Array = unary(array, |value| {
                    let seconds = value / 1000;
                    let nanoseconds = ((value % 1000) * 1_000_000) as u32;
    ...
    let return_type: ReturnTypeFunction = Arc::new(move |datatypes| {
        // Return type preserves input type
        let dtype = if in_local {
            datatypes[0].clone()
        } else {
            DataType::Date64
        };
...
fn make_timeunit_end_udf(units_mask: &[bool], in_local: bool) -> ScalarUDF {
    ...
    let timeunit_end = move |args: &[ArrayRef]| {
        ...
        let dtype = arg.data_type();
        let result_array = if matches!(dtype, DataType::Date64) {
            let start_array = arg.as_any().downcast_ref::<Date64Array>().unwrap();
...
fn perform_timeunit_start_from_in_local(value: i64, units_mask: &[bool]) -> i64 {
    ...
    let mut dt_value = date64_to_datetime(value).with_nanosecond(0).unwrap();
    ...
    if units_mask[1] {
        ...
        dt_value = NaiveDateTime::new(new_date, dt_value.time());
